...
The UI is still under construction but it's current state is available at [https://hotdoggies.stamer.demo.altostrat.com](https://hotdoggies.stamer.demo.altostrat.com).
...
The demo comes with a small client utility to demo streaming updates via the websocket connections. The utility opens a long-lasting connection with the API which will send back updates to dogs as they are processed in real-time.

```bash
export TOKEN="your-access-token"
cd clients/dogs/ && go run client.go
```
